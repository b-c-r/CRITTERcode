################################################################################
#   Example code to fit mod01r to data                                         #
#                                                                              #
...
source(paste(gh_path, f_path, "mod01r_rrpe_nll.R", sep = ""))                   # calculates negative log likelihood
source(paste(gh_path, f_path, "mod01r_rrpe_scan.R", sep = ""))                  # calculates a set negative log likelihoods (nll) of random parameters and returns parameters from lowest nll
source(paste(gh_path, f_path, "mod01r_rrpe_fit.R", sep = ""))                   # fits functional response model to data
...
################################################################################
## example for: mod01r_rrpe_nll.R

fit_ie_mod01r <- bbmle::mle2(                                                   # mle2 - the maximum likelihood estimator from the bbmle package
  minuslogl = mod01r_rrpe_nll,                                                  # mle2 requires the nll function for optimization
  start = list(
...
bbmle::summary(fit_ie_mod01r)                                                   # shows the summary table


################################################################################
## example for: mod01r_rrpe_scan.R
...
bbmle::AIC(mod01r_fit_ie)                                                       # shows the AIC of the best fit
BIC(mod01r_fit_ie)                                                              # shows the BIC of the best fit
