################################################################################
#   Example code to fit mod11h to data                                         #
#                                                                              #
...
source(paste(gh_path, f_path, "mod11h_rrpe_nll.R", sep = ""))                   # calculates negative log likelihood
source(paste(gh_path, f_path, "mod11h_rrpe_scan.R", sep = ""))                  # calculates a set negative log likelihoods (nll) of random parameters and returns parameters from lowest nll
source(paste(gh_path, f_path, "mod11h_rrpe_fit.R", sep = ""))                   # fits functional response model to data
...
################################################################################
## example for: mod11h_rrpe_nll.R

fit_ie_mod11h <- bbmle::mle2(                                                   # mle2 - the maximum likelihood estimator from the bbmle package
 minuslogl = mod11h_rrpe_nll,                                                   # mle2 requires the nll function for optimization
 start = list(
   ...
   a_intercept_log10    = -1.0,                                                 # start value for attack rate intercept (log10 scale, when habitat is absent)
   a_slope              =  0.0                                                  # start value for attack rate slope
 ),
 fixed = list(
...
bbmle::summary(fit_ie_mod11h)                                                   # shows the summary table


################################################################################
## example for: mod11h_rrpe_scan.R
...
mod11h_rrpe_scan(
 ...
 n_rings = fr_data_ie$ring_count,                                               # data: number of habitat rings provided as structure
 p = 1,                                                                         # number of predators in the experiment: here 1 predator per vessel
 t_h_intercept_log10_range = log10(c(.05, .05)),                                # two values, the range, for handling time intercept (log10 scale)
...
################################################################################
## example for: mod11h_rrpe_fit.R

mod11h_fit_ie <- mod11h_rrpe_fit(
 n_eaten = fr_data_ie$n_eaten,                                                  # data: number of prey eaten, as integer
 n_initial = fr_data_ie$n_initial,                                              # data: number of prey provided initially, as integer
 n_rings = fr_data_ie$ring_count                                                # data: number of habitat rings provided as structure
)

bbmle::summary(mod11h_fit_ie)                                                   # shows the summary table of the best fit
bbmle::AIC(mod11h_fit_ie)                                                       # shows the AIC of the best fit
BIC(mod11h_fit_ie)                                                              # shows the BIC of the best fit
