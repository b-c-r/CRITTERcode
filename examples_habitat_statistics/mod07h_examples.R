################################################################################
#   Example code to fit mod07h to data                                         #
#                                                                              #
...
source(paste(gh_path, f_path, "mod07h_rrpe_nll.R", sep = ""))                   # calculates negative log likelihood
source(paste(gh_path, f_path, "mod07h_rrpe_scan.R", sep = ""))                  # calculates a set negative log likelihoods (nll) of random parameters and returns parameters from lowest nll
source(paste(gh_path, f_path, "mod07h_rrpe_fit.R", sep = ""))                   # fits functional response model to data
...
################################################################################
## example for: mod07h_rrpe_nll.R

fit_ie_mod07h <- bbmle::mle2(                                                   # mle2 - the maximum likelihood estimator from the bbmle package
  minuslogl = mod07h_rrpe_nll,                                                  # mle2 requires the nll function for optimization
  start = list(
...
bbmle::summary(fit_ie_mod07h)                                                   # shows the summary table


################################################################################
## example for: mod07h_rrpe_scan.R
...
################################################################################
## example for: mod07h_rrpe_fit.R

mod07h_fit_ie <- mod07h_rrpe_fit(
  n_eaten = fr_data_ie$n_eaten,                                                 # data: number of prey eaten, as integer
  n_initial = fr_data_ie$n_initial,                                             # data: number of prey provided initially, as integer
  n_rings = fr_data_ie$ring_count,                                              # data: number of habitat structural elements (rings)
  complexity = fr_data_ie$complexity_level                                      # data: complexity levels
...
bbmle::summary(mod07h_fit_ie)                                                   # shows the summary table of the best fit
bbmle::AIC(mod07h_fit_ie)                                                       # shows the AIC of the best fit
BIC(mod07h_fit_ie)                                                              # shows the BIC of the best fit
