################################################################################
#   Example code to fit mod09h to data                                         #
#                                                                              #
...
source(paste(gh_path, f_path, "mod09h_rrpe_nll.R", sep = ""))                   # calculates negative log likelihood
source(paste(gh_path, f_path, "mod09h_rrpe_scan.R", sep = ""))                  # calculates a set negative log likelihoods (nll) of random parameters and returns parameters from lowest nll
source(paste(gh_path, f_path, "mod09h_rrpe_fit.R", sep = ""))                   # fits functional response model to data
...
################################################################################
## example for: mod09h_rrpe_nll.R

fit_ie_mod09h <- bbmle::mle2(                                                   # mle2 - the maximum likelihood estimator from the bbmle package
 minuslogl = mod09h_rrpe_nll,                                                   # mle2 requires the nll function for optimization
 start = list(
...
bbmle::summary(fit_ie_mod09h)                                                   # shows the summary table


################################################################################
## example for: mod09h_rrpe_scan.R
...
bbmle::summary(mod09h_fit_ie)                                                   # shows the summary table of the best fit
bbmle::AIC(mod09h_fit_ie)                                                       # shows the AIC of the best fit
BIC(mod09h_fit_ie)                                                              # shows the BIC of the best fit
