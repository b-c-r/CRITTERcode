################################################################################
#   Example code to fit mod16r to data                                         #
#                                                                              #
...
source(paste(gh_path, f_path, "mod16r_rrpe_nll.R", sep = ""))                   # calculates negative log likelihood
source(paste(gh_path, f_path, "mod16r_rrpe_scan.R", sep = ""))                  # calculates a set negative log likelihoods (nll) of random parameters and returns parameters from lowest nll
source(paste(gh_path, f_path, "mod16r_rrpe_fit.R", sep = ""))                   # fits functional response model to data
...
################################################################################
## example for: mod16r_rrpe_nll.R

fit_ie_mod16r <- bbmle::mle2(                                                   # mle2 - the maximum likelihood estimator from the bbmle package
  minuslogl = mod16r_rrpe_nll,                                                  # mle2 requires the nll function for optimization
  start = list(
...
bbmle::summary(fit_ie_mod16r)                                                   # shows the summary table


################################################################################
## example for: mod16r_rrpe_scan.R
...
mod16r_rrpe_scan(
  ...
  n_half_4_log10_range = log10(c(1, 1.5)),                                      # two values, the range, for half saturation density at complexity level 4 (log10 scale)
  t_end = 1,                                                                    # end time of the experiment, here 1 day
...
################################################################################
## example for: mod16r_rrpe_fit.R

mod16r_fit_ie <- mod16r_rrpe_fit(
  n_eaten = fr_data_ie$n_eaten,                                                 # data: number of prey eaten, as integer
  n_initial = fr_data_ie$n_initial,                                             # data: number of prey provided initially, as integer
  complexity = fr_data_ie$complexity_level                                      # data: complexity levels
)

bbmle::summary(mod16r_fit_ie)                                                   # shows the summary table of the best fit
bbmle::AIC(mod16r_fit_ie)                                                       # shows the AIC of the best fit
BIC(mod16r_fit_ie)                                                              # shows the BIC of the best fit
