...
source(paste(gh_path, f_path, "rrpe_nll_mod16r.R", sep = ""))                   # calculates negative log likelihood
source(paste(gh_path, f_path, "rrpe_scan_mod16r.R", sep = ""))                  # calculates a set negative log likelihoods (nll) of random parameters and returns parameters from lowest nll
source(paste(gh_path, f_path, "rrpe_fit_mod16r.R", sep = ""))                   # fits functional response model to data
...
################################################################################
## example for: rrpe_nll_mod16r.R

fit_ie_mod16r <- bbmle::mle2(                                                   # mle2 - the maximum likelihood estimator from the bbmle package
  minuslogl = rrpe_nll_mod16r,                                                  # mle2 requires the nll function for optimization
  start = list(
...
################################################################################
## example for: rrpe_scan_mod16r.R
