...
source(paste(gh_path, f_path, "mod01h_rrpe_nll.R", sep = ""))                   # calculates negative log likelihood
source(paste(gh_path, f_path, "mod01h_rrpe_scan.R", sep = ""))                  # calculates a set negative log likelihoods (nll) of random parameters and returns parameters from lowest nll
source(paste(gh_path, f_path, "mod01h_rrpe_fit.R", sep = ""))                   # fits functional response model to data
...
################################################################################
## example for: mod01h_rrpe_nll.R

fit_ie_mod01h <- bbmle::mle2(                                                   # mle2 - the maximum likelihood estimator from the bbmle package
 minuslogl = mod01h_rrpe_nll,                                                   # mle2 requires the nll function for optimization
 start = list(
...
bbmle::summary(fit_ie_mod01h)                                                   # shows the summary table


################################################################################
## example for: mod01h_rrpe_scan.R
...
################################################################################
## example for: mod01h_rrpe_fit.R

mod01h_fit_ie <- mod01h_rrpe_fit(
 n_eaten = fr_data_ie$n_eaten,                                                  # data: number of prey eaten, as integer
 n_initial = fr_data_ie$n_initial                                               # data: number of prey provided initially, as integer
)

bbmle::summary(mod01h_fit_ie)                                                   # shows the summary table of the best fit
bbmle::AIC(mod01h_fit_ie)                                                       # shows the AIC of the best fit
BIC(mod01h_fit_ie)                                                              # shows the BIC of the best fit
