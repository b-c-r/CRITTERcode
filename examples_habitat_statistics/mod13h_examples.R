...
source(paste(gh_path, f_path, "mod13h_rrpe_nll.R", sep = ""))                   # calculates negative log likelihood
source(paste(gh_path, f_path, "mod13h_rrpe_scan.R", sep = ""))                  # calculates a set negative log likelihoods (nll) of random parameters and returns parameters from lowest nll
source(paste(gh_path, f_path, "mod13h_rrpe_fit.R", sep = ""))                   # fits functional response model to data
...
################################################################################
## example for: mod13h_rrpe_nll.R

fit_ie_mod13h <- bbmle::mle2(                                                   # mle2 - the maximum likelihood estimator from the bbmle package
 minuslogl = mod13h_rrpe_nll,                                                   # mle2 requires the nll function for optimization
 start = list(
...
################################################################################
## example for: mod13h_rrpe_scan.R
...
################################################################################
## example for: mod13h_rrpe_fit.R

mod13h_fit_ie <- mod13h_rrpe_fit(
 n_eaten = fr_data_ie$n_eaten,                                                  # data: number of prey eaten, as integer
 n_initial = fr_data_ie$n_initial,                                              # data: number of prey provided initially, as integer
 complexity = fr_data_ie$complexity_level                                       # data: complexity levels
)
