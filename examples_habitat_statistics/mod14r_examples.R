...
source(paste(gh_path, f_path, "rrpe_nll_mod14r.R", sep = ""))                   # calculates negative log likelihood
source(paste(gh_path, f_path, "rrpe_scan_mod14r.R", sep = ""))                  # calculates a set negative log likelihoods (nll) of random parameters and returns parameters from lowest nll
source(paste(gh_path, f_path, "rrpe_fit_mod14r.R", sep = ""))                   # fits functional response model to data
...
################################################################################
## example for: rrpe_nll_mod14r.R

fit_ie_mod14r <- bbmle::mle2(                                                   # mle2 - the maximum likelihood estimator from the bbmle package
 minuslogl = rrpe_nll_mod14r,                                                   # mle2 requires the nll function for optimization
 start = list(
...
################################################################################
## example for: rrpe_scan_mod14r.R
...
################################################################################
## example for: rrpe_fit_mod14r.R

mod14r_fit_ie <- rrpe_fit_mod14r(
 n_eaten = fr_data_ie$n_eaten,                                                  # data: number of prey eaten, as integer
 n_initial = fr_data_ie$n_initial,                                              # data: number of prey provided initially, as integer
 complexity = fr_data_ie$complexity_level                                       # data: complexity levels
)
