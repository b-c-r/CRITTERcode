################################################################################
#   rrpe_parms_scan_mod14r: scans for reasonable starting parameters           #
#                                                                              #
...
################################################################################
#'
#' @description `rrpe_parms_scan_mod14r` creates Latin hypercube samples for the
...
#'     `rrpe_parms_scan_mod14r` requires the lhs package (Carnell 2024).
#' 
...
#' source(paste(gh_path, "rrpe_nll_mod14r.R", sep = ""))
#' source(paste(gh_path, "rrpe_scan_mod14r.R", sep = ""))
#' 
...
#'   n_half_range_hab0_log10 = log10(c(1, max(fr_data_ie$n_initial[fr_data_ie$complexity_level == 0]))),
#'   n_half_range_hab1_log10 = log10(c(1, max(fr_data_ie$n_initial[fr_data_ie$complexity_level  > 0]))),
#'   t_end = 1,
#'   no_lhs_samples = 100
#' )
#' 
...
#'   n_half_range_hab0_log10 = log10(c(1, max(fr_data_ng$n_initial[fr_data_ng$complexity_level == 0]))),
#'   n_half_range_hab1_log10 = log10(c(1, max(fr_data_ng$n_initial[fr_data_ng$complexity_level  > 0]))),
#'   t_end = 1,
#'   no_lhs_samples = 100
#' )
#' 

rrpe_scan_mod14r <- function(
    n_eaten,
    n_initial,
    complexity,
    p = 1,
    ...
    f_max_range_3_log10,
    f_max_range_4_log10,
    n_half_range_hab0_log10,
    n_half_range_hab1_log10,
    t_end = 1,
    no_lhs_samples = 1000
){
  ...
  n_half_range_hab0_log10  <- log10((lhsvals[, 6] * (10^n_half_range_hab0_log10[2] - 10^n_half_range_hab0_log10[1])) + 10^n_half_range_hab0_log10[1])
  n_half_range_hab1_log10  <- log10((lhsvals[, 7] * (10^n_half_range_hab1_log10[2] - 10^n_half_range_hab1_log10[1])) + 10^n_half_range_hab1_log10[1])
  ...
  nlls <- foreach::foreach(
    ...
    .combine = "c") %do% {
      ...
      nll <- rrpe_nll_mod14r(
          ...
          n_half_hab0_log10 = n_half_range_hab0_log10[i],
          n_half_hab1_log10 = n_half_range_hab1_log10[i],
          t_end = t_end
        )
  ...
  sel_parms <- data.frame(
    ...
    n_half_hab0_log10 = n_half_range_hab0_log10[nlls == min(nlls)],
    n_half_hab1_log10 = n_half_range_hab1_log10[nlls == min(nlls)],
