################################################################################
#   rrpe_nll_mod14r: estimates the negative log-likelihood of Model 16r        #
#                                                                              #
#    Copyright (C) 2025                                                        #
...
#' 
#'
#' @description `rrpe_nll_mod14r` calculates the negative log likelihood of
...
#' @param n_half_hab0_log10 n_half for if habitat is absent, single value.
#' @param n_half_hab1_log10 n_half for if habitat is present, single value.
...
#'     n_half_hab0_log10 = log10(mean(fr_data_ie$n_initial[fr_data_ie$complexity_level == 0])),
#'     n_half_hab1_log10 = log10(mean(fr_data_ie$n_initial[fr_data_ie$complexity_level  > 0]))
#'   ),
#'   fixed = list(
#'     t_end = 1,
#'     p = 1
...
#' )
#' 
#' bbmle::summary(fit_ie_mod14r)
#' 
#' #############################################################################
#' 
#' fr_data_ng <- subset(fr_data, predator == "Notonecta glauca")
#' 
#' fit_ng_mod14r <- bbmle::mle2(
#'   minuslogl = rrpe_nll_mod14r,
#'   start = list(
...
#'     n_half_hab0_log10 = log10(mean(fr_data_ng$n_initial[fr_data_ng$complexity_level == 0])),
#'     n_half_hab1_log10 = log10(mean(fr_data_ng$n_initial[fr_data_ng$complexity_level  > 0]))
#'   ),
#'   fixed = list(
#'     t_end = 1,
#'     p = 1
...
#' )
#' 
#' bbmle::summary(fit_ng_mod14r)
#' 

rrpe_nll_mod14r <- function(
    n_eaten,
    n_initial,
    complexity,
    p = 1,
  ...
  try({
    eaten_simulated <- foreach::foreach(
      ...
      if(complexity[i] == 0){
        ...
        n_half_log10 <- n_half_hab0_log10
      }
      if(complexity[i] > 0){
        n_half_log10 <- n_half_hab1_log10
      }
      if(complexity[i] == 1){
