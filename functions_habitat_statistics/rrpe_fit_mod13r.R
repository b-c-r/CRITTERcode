################################################################################
#   rrpe_fit_mod13r: fitting the RRPE to data                                  #
#                                                                              #
...
################################################################################
#'
#' @description `rrpe_fit_mod13r` allows for fitting the Rogers' Random Predator
...
#'
#' @include rrpe_sim.R
#' @include rrpe_nll_mod13r.R
#' @include rrpe_scan_mod13r.R
#'
...
#' source(paste(gh_path, "rrpe_nll_mod13r.R", sep = ""))
#' source(paste(gh_path, "rrpe_scan_mod13r.R", sep = ""))
#' source(paste(gh_path, "rrpe_fit_mod13r.R", sep = ""))
#'
#' ################################################################################
#' ## Data
#' fr_data <- read.csv(
...
rrpe_fit_mod13r <- function(
  ...
  initial_guess <- rrpe_scan_mod13r(
    n_eaten = n_eaten,
    n_initial = n_initial,
  ...
  message(paste("n_half_log10: ", round(bbmle::coef(fit[[1]])[7],2)))
  ...
  while(witer <= witer_max){
    ...
    start_parms <- foreach::foreach(
      ...
      .combine = "rbind") %do% {
        ...
        n_half_range <- c(10^bbmle::coef(fit[[witer-1]])[7]  / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[7]  * range_multiplier[i])
    ...
    message(paste("n_half_log10: ", round(bbmle::coef(fit[[witer]])[7],2)))
