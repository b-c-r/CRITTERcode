################################################################################
#   mod02r_rrpe_fit: fitting the RRPE to data                                  #
#                                                                              #
...
#' 
#' @include rrpe_sim.R
#' @include mod02r_rrpe_nll.R
#' @include mod02r_rrpe_scan.R
#' 
...
#' # https://github.com/b-c-r/CRITTERcode/examples_habitat_statistics/examples_habitat_statistics/mod02r_examples.R
#' 

mod02r_rrpe_fit <- function(
    n_eaten,
    n_initial,
    complexity,
    p = 1,
  ...
  initial_guess <- mod02r_rrpe_scan(
    n_eaten = n_eaten,
    n_initial = n_initial,
  ...
  while(witer <= witer_max){
    ...
    start_parms <- foreach::foreach(
      ...
      .combine = "rbind") %do% {
        ...
        n_half_hab0_range <- c(10^bbmle::coef(fit[[witer-1]])[3] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[3] * range_multiplier[i])
        n_half_hab1_range <- c(10^bbmle::coef(fit[[witer-1]])[4] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[4] * range_multiplier[i])
    ...
    message(paste("n_half_hab0_log10: ", round(bbmle::coef(fit[[witer]])[3],2)))
    message(paste("n_half_hab1_log10: ", round(bbmle::coef(fit[[witer]])[4],2)))
