################################################################################
#   mod04h_rrpe_fit: fitting the RRPE to data                                  #
#                                                                              #
...
#' 
#' @include rrpe_sim.R
#' @include mod04h_rrpe_nll.R
#' @include mod04h_rrpe_scan.R
#' 
...
#' # https://github.com/b-c-r/CRITTERcode/examples_habitat_statistics/examples_habitat_statistics/mod04h_examples.R
#' 

mod04h_rrpe_fit <- function(
    n_eaten,
    n_initial,
    n_rings = NA,
    complexity,
  ...
  initial_guess <- mod04h_rrpe_scan(
    n_eaten = n_eaten,
    n_initial = n_initial,
    complexity  = complexity,
    p = p,
    t_h_log10_range = log10(1/rev(f_max_hab0_range)),
    a_0_log10_range = log10(c(f_max_hab0_range[1]/n_half_0_range[2], f_max_hab0_range[2]/n_half_0_range[1])),
    a_1_log10_range = log10(c(f_max_hab1_range[1]/n_half_1_range[2], f_max_hab1_range[2]/n_half_1_range[1])),
  ...
  fit[[1]] <- bbmle::mle2(
    minuslogl = mod04h_rrpe_nll,
    start = list(
      t_h_log10  =  initial_guess$t_h_log10,
  ...
  message(paste("t_h_log10: ", round(bbmle::coef(fit[[1]])[2],2)))
  message(paste("a_0_log10: ", round(bbmle::coef(fit[[1]])[3],2)))
  message(paste("a_1_log10: ", round(bbmle::coef(fit[[1]])[4],2)))
  message(paste("a_2_log10: ", round(bbmle::coef(fit[[1]])[5],2)))
  message(paste("a_3_log10: ", round(bbmle::coef(fit[[1]])[6],2)))
  message(paste("a_4_log10: ", round(bbmle::coef(fit[[1]])[7],2)))
  ...
  while(witer <= witer_max){
    ...
    start_parms <- foreach::foreach(
      ...
      .combine = "rbind") %do% {
        t_h_range <- c(10^bbmle::coef(fit[[witer-1]])[2] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[2] * range_multiplier[i])
        a_0_range <- c(10^bbmle::coef(fit[[witer-1]])[3] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[3] * range_multiplier[i])
        a_1_range <- c(10^bbmle::coef(fit[[witer-1]])[4] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[4] * range_multiplier[i])
        a_2_range <- c(10^bbmle::coef(fit[[witer-1]])[5] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[5] * range_multiplier[i])
        a_3_range <- c(10^bbmle::coef(fit[[witer-1]])[6] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[6] * range_multiplier[i])
        a_4_range <- c(10^bbmle::coef(fit[[witer-1]])[7] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[7] * range_multiplier[i])
        ...
        mod04h_rrpe_scan(
          ...
          complexity  = complexity,
          p = p,
          t_h_log10_range = log10(t_h_range),
    ...
    fit[[witer]] <- bbmle::mle2(
      minuslogl = mod04h_rrpe_nll,
      start = list(
        t_h_log10 = start_parms$t_h_log10,
    ...
    message(paste("t_h_log10: ", round(bbmle::coef(fit[[witer]])[2],2)))
    message(paste("a_0_log10: ", round(bbmle::coef(fit[[witer]])[3],2)))
    message(paste("a_1_log10: ", round(bbmle::coef(fit[[witer]])[4],2)))
    message(paste("a_2_log10: ", round(bbmle::coef(fit[[witer]])[5],2)))
    message(paste("a_3_log10: ", round(bbmle::coef(fit[[witer]])[6],2)))
    message(paste("a_4_log10: ", round(bbmle::coef(fit[[witer]])[7],2)))
