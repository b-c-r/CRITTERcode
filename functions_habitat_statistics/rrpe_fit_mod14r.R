################################################################################
#   rrpe_fit_mod14r: fitting the RRPE to data                                  #
#                                                                              #
...
################################################################################
#'
#' @description `rrpe_fit_mod14r` allows for fitting the Rogers' Random Predator
...
#' source(paste(gh_path, "rrpe_fit_mod14r.R", sep = ""))
#'
#' ################################################################################
#' ## Data
#' fr_data <- read.csv(
...
rrpe_fit_mod14r <- function(
  ...
  initial_guess <- rrpe_scan_mod14r(
    n_eaten = n_eaten,
    n_initial = n_initial,
    ...
    n_half_range_hab0_log10 = log10(n_half_range_hab0),
    n_half_range_hab1_log10 = log10(n_half_range_hab1),
    t_end = t_end,
  ...
  message(paste("n_half_hab0_log10: ", round(bbmle::coef(fit[[1]])[7],2)))
  message(paste("n_half_hab1_log10: ", round(bbmle::coef(fit[[1]])[8],2)))
  ...
  while(witer <= witer_max){
    ...
    start_parms <- foreach::foreach(
      ...
      .combine = "rbind") %do% {
        ...
        n_half_range_hab0 <- c(10^bbmle::coef(fit[[witer-1]])[7]  / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[7]  * range_multiplier[i])
        n_half_range_hab1 <- c(10^bbmle::coef(fit[[witer-1]])[8]  / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[8]  * range_multiplier[i])
    ...
    message(paste("n_half_hab0_log10: ", round(bbmle::coef(fit[[witer]])[7],2)))
    message(paste("n_half_hab1_log10: ", round(bbmle::coef(fit[[witer]])[8],2)))
