################################################################################
#   mod03r_rrpe_fit: fitting the RRPE to data                                  #
#                                                                              #
...
mod03r_rrpe_fit <- function(
  ...
  message(paste("n_half_intercept_log10: ", round(bbmle::coef(fit[[1]])[3],2)))
  message(paste("n_half_slope: "    , round(bbmle::coef(fit[[1]])[4],2)))
  ...
  while(witer <= witer_max){
    ...
    start_parms <- foreach::foreach(
      ...
      .combine = "rbind") %do% {
        ...
        n_half_range_intercept <- c(10^bbmle::coef(fit[[witer-1]])[3] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[3] * range_multiplier[i])
        ...
        mod03r_rrpe_scan(
          ...
          n_half_slope_range = n_half_range_slope,
          t_end = t_end,
    ...
    message(paste("n_half_intercept_log10: ", round(bbmle::coef(fit[[witer]])[3],2)))
    message(paste("n_half_slope: "    , round(bbmle::coef(fit[[witer]])[4],2)))
