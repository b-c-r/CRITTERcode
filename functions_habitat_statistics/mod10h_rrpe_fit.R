################################################################################
#   mod10h_rrpe_fit: fitting the RRPE to data                                  #
#                                                                              #
...
#' 
#' @include rrpe_sim.R
#' @include mod10h_rrpe_nll.R
#' @include mod10h_rrpe_scan.R
#' 
...
#' # https://github.com/b-c-r/CRITTERcode/examples_habitat_statistics/examples_habitat_statistics/mod10h_examples.R
#' 

mod10h_rrpe_fit <- function(
    n_eaten,
    n_initial,
    n_rings,
    complexity,
  ...
  initial_guess <- mod10h_rrpe_scan(
    n_eaten = n_eaten,
    n_initial = n_initial,
    n_rings = n_rings,
    complexity  = complexity,
    p = p,
    t_h_intercept_log10_range = log10(1/rev(f_max_hab0_range)),
    t_h_slope_range = slope_range,
  ...
  message(paste("t_h_slope: "          , round(bbmle::coef(fit[[1]])[3],2)))
  message(paste("a_hab0_log10: ", round(bbmle::coef(fit[[1]])[4],2)))
  message(paste("a_hab1_log10: ", round(bbmle::coef(fit[[1]])[5],2)))
  ...
  while(witer <= witer_max){
    ...
    start_parms <- foreach::foreach(
      ...
      .combine = "rbind") %do% {
        ...
        a_hab0_range <- c(10^bbmle::coef(fit[[witer-1]])[4]  / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[4]  * range_multiplier[i])
        a_hab1_range <- c(10^bbmle::coef(fit[[witer-1]])[5]  / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[5]  * range_multiplier[i])
    ...
    message(paste("t_h_slope: ",      round(bbmle::coef(fit[[witer]])[3],2)))
    message(paste("a_hab0_log10: ", round(bbmle::coef(fit[[witer]])[4],2)))
    message(paste("a_hab1_log10: ", round(bbmle::coef(fit[[witer]])[5],2)))
