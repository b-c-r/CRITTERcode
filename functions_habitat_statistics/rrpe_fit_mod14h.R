################################################################################
#   rrpe_fit_mod14h: fitting the RRPE to data                                  #
#                                                                              #
...
################################################################################
#'
#' @description `rrpe_fit_mod14h` allows for fitting the Rogers' Random Predator
...
#' source(paste(gh_path, "rrpe_nll_mod14h.R", sep = ""))
#' source(paste(gh_path, "rrpe_scan_mod14h.R", sep = ""))
#' source(paste(gh_path, "rrpe_fit_mod14h.R", sep = ""))
#'
#' ################################################################################
#' ## Data
#' fr_data <- read.csv(
...
rrpe_fit_mod14h <- function(
  ...
  n_half_range_hab0 <- rel_n_half_range * max(n_initial[complexity == 0])
  n_half_range_hab1 <- rel_n_half_range * max(n_initial[complexity  > 0])
  f_max_range_hab1  <- rel_f_max_range  *    max(n_eaten[complexity > 0])/t_end
  
  # initial lhs sampling
  initial_guess <- rrpe_scan_mod14h(
    n_eaten = n_eaten,
    n_initial = n_initial,
    ...
    a_range_hab0_log10 = log10(c(f_max_range_0[1]/n_half_range_hab0[2], f_max_range_0[2]/n_half_range_hab0[1])),
    a_range_hab1_log10 = log10(c(f_max_range_hab1[1]/n_half_range_hab1[2], f_max_range_hab1[2]/n_half_range_hab1[1])),
    t_end = t_end,
  ...
  fit[[1]] <- bbmle::mle2(
    ...
    start = list(
      ...
      a_hab0_log10 = initial_guess$a_hab0_log10,
      a_hab1_log10 = initial_guess$a_hab1_log10
    ),
    fixed = list(
      t_end = t_end,
      p = p
  ...
  message(paste("a_hab0_log10: ", round(bbmle::coef(fit[[1]])[7],2)))
  message(paste("a_hab1_log10: ", round(bbmle::coef(fit[[1]])[8],2)))
  ...
  while(witer <= witer_max){
    ...
    start_parms <- foreach::foreach(
      ...
      .combine = "rbind") %do% {
        ...
        a_range_hab0 <- c(10^bbmle::coef(fit[[witer-1]])[7]  / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[7]  * range_multiplier[i])
        a_range_hab1 <- c(10^bbmle::coef(fit[[witer-1]])[8]  / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[8]  * range_multiplier[i])
        ...
        rrpe_scan_mod14h(
          ...
          a_range_hab0_log10 = log10(a_range_hab0),
          a_range_hab1_log10 = log10(a_range_hab1),
          t_end = t_end,
    ...
    fit[[witer]] <- bbmle::mle2(
      ...
      start = list(
        ...
        a_hab0_log10 = start_parms$a_hab0_log10,
        a_hab1_log10 = start_parms$a_hab1_log10
      ),
      fixed = list(
        t_end = t_end,
        p = p
    ...
    message(paste("a_hab0_log10: ", round(bbmle::coef(fit[[witer]])[7],2)))
    message(paste("a_hab1_log10: ", round(bbmle::coef(fit[[witer]])[8],2)))
