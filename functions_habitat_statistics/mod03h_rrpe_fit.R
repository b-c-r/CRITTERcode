################################################################################
#   mod03h_rrpe_fit: fitting the RRPE to data                                  #
#                                                                              #
...
#' 
#' @include rrpe_sim.R
#' @include mod03h_rrpe_nll.R
#' @include mod03h_rrpe_scan.R
#' 
...
#' # https://github.com/b-c-r/CRITTERcode/examples_habitat_statistics/examples_habitat_statistics/mod03h_examples.R
#' 

mod03h_rrpe_fit <- function(
    n_eaten,
    n_initial,
    n_rings,
    p = 1,
  ...
  initial_guess <- mod03h_rrpe_scan(
    n_eaten = n_eaten,
    n_initial = n_initial,
    n_rings = n_rings,
    p = p,
    t_h_log10_range = log10(1/rev(f_max_0_range)),
    a_intercept_log10_range = log10(c(f_max_0_range[1]/n_half_0_range[2], f_max_0_range[2]/n_half_0_range[1])),
  ...
  message(paste("a_intercept_log10: ", round(bbmle::coef(fit[[1]])[3],2)))
  message(paste("a_slope: "          , round(bbmle::coef(fit[[1]])[4],2)))
  ...
  while(witer <= witer_max){
    ...
    start_parms <- foreach::foreach(
      ...
      .combine = "rbind") %do% {
        ...
        a_intercept_range <- c(10^bbmle::coef(fit[[witer-1]])[3] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[3] * range_multiplier[i])
        a_slope_range     <- c(
          bbmle::coef(fit[[witer-1]])[4] - range_multiplier[i] * abs(bbmle::coef(fit[[witer-1]])[4]),
          bbmle::coef(fit[[witer-1]])[4] + range_multiplier[i] * abs(bbmle::coef(fit[[witer-1]])[4])
        )
    ...
    message(paste("a_intercept_log10: ", round(bbmle::coef(fit[[witer]])[3],2)))
    message(paste("a_slope: "          , round(bbmle::coef(fit[[witer]])[4],2)))
