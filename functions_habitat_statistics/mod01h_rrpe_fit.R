################################################################################
#   mod01h_rrpe_fit: fitting the RRPE to data                                  #
#                                                                              #
...
#' # https://github.com/b-c-r/CRITTERcode/examples_habitat_statistics/examples_habitat_statistics/mod01h_examples.R
#' 

mod01h_rrpe_fit <- function(
    n_eaten,
    n_initial,
    n_rings = NA,
    complexity = NA,
  ...
  initial_guess <- mod01h_rrpe_scan(
    n_eaten = n_eaten,
    n_initial = n_initial,
    p = p,
  ...
  fit[[1]] <- bbmle::mle2(
    minuslogl = mod01h_rrpe_nll,
    start = list(
      t_h_log10 = initial_guess$t_h_log10,
      a_log10   = initial_guess$a_log10
    ),
    fixed = list(
      t_end = t_end,
      p = p
  ...
  message(paste("a_log10: ", round(bbmle::coef(fit[[1]])[3],2)))
  ...
  while(witer <= witer_max){
    ...
    start_parms <- foreach::foreach(
      ...
      .combine = "rbind") %do% {
        ...
        a_range   <- c(10^bbmle::coef(fit[[witer-1]])[3] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[3] * range_multiplier[i])
    ...
    message(paste("a_log10: "  , round(bbmle::coef(fit[[witer]])[3],2)))
