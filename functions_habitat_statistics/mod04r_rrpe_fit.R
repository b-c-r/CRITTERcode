################################################################################
#   mod04r_rrpe_fit: fitting the RRPE to data                                  #
#                                                                              #
...
#' 
#' @include rrpe_sim.R
#' @include mod04r_rrpe_nll.R
#' @include mod04r_rrpe_scan.R
#' 
...
#' # https://github.com/b-c-r/CRITTERcode/examples_habitat_statistics/examples_habitat_statistics/mod04r_examples.R
#' 

mod04r_rrpe_fit <- function(
    n_eaten,
    n_initial,
    n_rings = NA,
    complexity,
  ...
  initial_guess <- mod04r_rrpe_scan(
    n_eaten = n_eaten,
    n_initial = n_initial,
  ...
  message(paste("n_half_0_log10: ", round(bbmle::coef(fit[[1]])[3],2)))
  message(paste("n_half_1_log10: ", round(bbmle::coef(fit[[1]])[4],2)))
  message(paste("n_half_2_log10: ", round(bbmle::coef(fit[[1]])[5],2)))
  message(paste("n_half_3_log10: ", round(bbmle::coef(fit[[1]])[6],2)))
  message(paste("n_half_4_log10: ", round(bbmle::coef(fit[[1]])[7],2)))
  ...
  while(witer <= witer_max){
    ...
    start_parms <- foreach::foreach(
      ...
      .combine = "rbind") %do% {
        ...
        n_half_0_range <- c(10^bbmle::coef(fit[[witer-1]])[3] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[3] * range_multiplier[i])
        n_half_1_range <- c(10^bbmle::coef(fit[[witer-1]])[4] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[4] * range_multiplier[i])
        n_half_2_range <- c(10^bbmle::coef(fit[[witer-1]])[5] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[5] * range_multiplier[i])
        n_half_3_range <- c(10^bbmle::coef(fit[[witer-1]])[6] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[6] * range_multiplier[i])
        n_half_4_range <- c(10^bbmle::coef(fit[[witer-1]])[7] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[7] * range_multiplier[i])
    ...
    message(paste("n_half_0_log10: " , round(bbmle::coef(fit[[witer]])[3],2)))
    message(paste("n_half_1_log10: " , round(bbmle::coef(fit[[witer]])[4],2)))
    message(paste("n_half_2_log10: " , round(bbmle::coef(fit[[witer]])[5],2)))
    message(paste("n_half_3_log10: " , round(bbmle::coef(fit[[witer]])[6],2)))
    message(paste("n_half_4_log10: " , round(bbmle::coef(fit[[witer]])[7],2)))
