################################################################################
#   rrpe_nll_mod15h: estimates the negative log-likelihood of Model 16r        #
#                                                                              #
#    Copyright (C) 2025                                                        #
...
#' 
#'
#' @description `rrpe_nll_mod15h` calculates the negative log likelihood of
...
#' @param a_intercept_log10 float; intercept of log10 a, single value.
#' @param a_slope_log10 float; slope of log10 a, single value.
...
#'     a_intercept_log10 = log10(mean(fr_data_ie$n_initial[fr_data_ie$ring_count == 0])),
#'     a_slope_log10 = 0
#'   ),
#'   fixed = list(
#'     t_end = 1,
...
#' )
#' 
#' bbmle::summary(fit_ie_mod15h)
#' 
#' #############################################################################
#' 
#' fr_data_ng <- subset(fr_data, predator == "Notonecta glauca")
#' 
#' fit_ng_mod15h <- bbmle::mle2(
#'   minuslogl = rrpe_nll_mod15h,
#'   start = list(
...
#'     a_intercept_log10 = log10(mean(fr_data_ng$n_initial[fr_data_ng$ring_count == 0])),
#'     a_slope_log10 = 0
#'   ),
#'   fixed = list(
#'     t_end = 1,
...
#' )
#' 
#' bbmle::summary(fit_ng_mod15h)
#' 

rrpe_nll_mod15h <- function(
    n_eaten,
    n_initial,
    n_rings,
    complexity,
  ...
  eaten_simulated <- foreach::foreach(
    ...
    rrpe_sim(
      ...
      n_half = 1/(10^(a_intercept_log10 + a_slope_log10 * n_rings[i]) * 10^t_h_log10),
      t_end = t_end
    )
  }
