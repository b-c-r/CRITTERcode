...
#' 
#'
#' @description `rrpe_nll_mod14h` calculates the negative log likelihood of
...
#' @param a_hab0_log10 a if habitat is absent, single value.
#' @param a_hab1_log10 a if habitat is present, single value.
...
#'     a_hab0_log10 = log10(mean(fr_data_ie$n_initial[fr_data_ie$complexity_level == 0])),
#'     a_hab1_log10 = log10(mean(fr_data_ie$n_initial[fr_data_ie$complexity_level  > 0]))
#'   ),
#'   fixed = list(
#'     t_end = 1,
#'     p = 1
...
#' )
#' 
#' bbmle::summary(fit_ie_mod14h)
#' 
#' #############################################################################
#' 
#' fr_data_ng <- subset(fr_data, predator == "Notonecta glauca")
#' 
#' fit_ng_mod14h <- bbmle::mle2(
#'   minuslogl = rrpe_nll_mod14h,
#'   start = list(
...
#'     a_hab0_log10 = log10(mean(fr_data_ng$n_initial[fr_data_ng$complexity_level == 0])),
#'     a_hab1_log10 = log10(mean(fr_data_ng$n_initial[fr_data_ng$complexity_level  > 0]))
#'   ),
#'   fixed = list(
#'     t_end = 1,
#'     p = 1
...
#' )
#' 
#' bbmle::summary(fit_ng_mod14h)
#' 

rrpe_nll_mod14h <- function(
    n_eaten,
    n_initial,
    complexity,
    p = 1,
  ...
  try({
    eaten_simulated <- foreach::foreach(
      ...
      if(complexity[i] == 0){
        ...
        a_log10 <- a_hab0_log10
      }
      if(complexity[i] > 0){
        a_log10 <- a_hab1_log10
      }
      if(complexity[i] == 1){
