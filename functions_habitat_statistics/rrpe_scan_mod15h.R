################################################################################
#   rrpe_parms_scan_mod15h: scans for reasonable starting parameters           #
#                                                                              #
...
################################################################################
#'
#' @description `rrpe_parms_scan_mod15h` creates Latin hypercube samples for the
...
#'     `rrpe_parms_scan_mod15h` requires the lhs package (Carnell 2024).
#' 
...
#' @param a_range_intercept_log10 float; intercept of log10 a, two values.
#' @param a_range_slope_log10 float; slope of log10 a, two values.
...
#'   a_range_intercept_log10 = log10(c(1, max(fr_data_ie$n_initial[fr_data_ie$ring_count == 0]))),
#'   a_range_slope_log10 = c(-0.1, 0.1),
#'   t_end = 1,
#'   no_lhs_samples = 100
#' )
#' 
...
#'   a_range_intercept_log10 = log10(c(1, max(fr_data_ng$n_initial[fr_data_ng$ring_count == 0]))),
#'   a_range_slope_log10 = c(-0.1, 0.1),
#'   t_end = 1,
#'   no_lhs_samples = 100
#' )
...
rrpe_scan_mod15h <- function(
  ...
  nlls <- foreach::foreach(
    ...
    .combine = "c") %do% {
      ...
      nll <- rrpe_nll_mod15h(
          ...
          a_intercept_log10 = a_range_intercept_log10[i],
          a_slope_log10 = a_range_slope_log10[i],
          t_end = t_end
        )
  ...
  sel_parms <- data.frame(
    ...
    a_intercept_log10 = a_range_intercept_log10[nlls == min(nlls)],
    a_slope_log10 = a_range_slope_log10[nlls == min(nlls)],
