################################################################################
#   mod07r_rrpe_fit: fitting the RRPE to data                                  #
#                                                                              #
...
mod07r_rrpe_fit <- function(
  ...
  initial_guess <- mod07r_rrpe_scan(
    n_eaten = n_eaten,
    n_initial = n_initial,
  ...
  while(witer <= witer_max){
    ...
    start_parms <- foreach::foreach(
      ...
      .combine = "rbind") %do% {
        ...
        n_half_range_intercept <- c(10^bbmle::coef(fit[[witer-1]])[4] / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[4] * range_multiplier[i])
