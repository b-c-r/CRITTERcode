################################################################################
#   mod09h_rrpe_fit: fitting the RRPE to data                                  #
#                                                                              #
...
#' 
#' @include rrpe_sim.R
#' @include mod09h_rrpe_nll.R
#' @include mod09h_rrpe_scan.R
#' 
...
#' # https://github.com/b-c-r/CRITTERcode/examples_habitat_statistics/examples_habitat_statistics/mod09h_examples.R
#' 

mod09h_rrpe_fit <- function(
    n_eaten,
    n_initial,
    n_rings,
    complexity,
  ...
  initial_guess <- mod09h_rrpe_scan(
    n_eaten = n_eaten,
    n_initial = n_initial,
    n_rings = n_rings,
    complexity  = complexity,
    p = p,
    t_h_intercept_log10_range = log10(1/rev(f_max_0_range)),
  ...
  message(paste("a_log10: ", round(bbmle::coef(fit[[1]])[4],2)))
  ...
  while(witer <= witer_max){
    ...
    start_parms <- foreach::foreach(
      ...
      .combine = "rbind") %do% {
        ...
        a_range <- c(10^bbmle::coef(fit[[witer-1]])[4]  / range_multiplier[i], 10^bbmle::coef(fit[[witer-1]])[4]  * range_multiplier[i])
